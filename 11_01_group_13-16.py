...
OUT_DIR = Path("out/11_01_groups_13_16")

# Fixed layout based on periodic table positions: 5 rows × 4 columns
# The columns correspond to groups 13, 14, 15, and 16
PERIODIC_LAYOUT = [
    ["B",  "C",  "N",  "O"],
    ["Al", "Si", "P",  "S"],
    ["Ga", "Ge", "As", "Se"],
...
PBLOCK_ELEMENTS = [el for row in PERIODIC_LAYOUT for el in row]
...
#   "equal" -> each CN line has the same weight
#   "n"     -> weighted by the sample count of each CN
LSP_WEIGHT_MODE = "equal"

# Plot style
FIG_DPI = 260
POINTS_BG = False            # whether to show raw scatter points in the background
POINT_ALPHA = 0.18
POINT_SIZE = 7
...
def detect_coord_column(df: pd.DataFrame, element: str):
    """
    First look for columns such as:
        AlO_coordn, SiO_coordn, ...
    If not found, fall back to any column ending with O_coordn
    """
...
def plot_pblock_elements_periodic_layout(method, all_points_dict, all_lines_df, all_lsp_df, out_fig):
    """
    Plot using fixed periodic-table positions:
    5 rows × 4 columns
    Each position corresponds to one fixed element
    """
    nrows = len(PERIODIC_LAYOUT)
    ncols = len(PERIODIC_LAYOUT[0])
    ...
    # Ensure axes[row, col] can be indexed directly
    axes = np.array(axes).reshape(nrows, ncols)

    for r in range(nrows):
        for c in range(ncols):
            ax = axes[r, c]
            element = PERIODIC_LAYOUT[r][c]
            color = element_color(element)

            ax.set_title(f"{element}-O", fontsize=12)

            # If this element was not successfully loaded, keep the slot
            # so that the layout still matches the periodic table
    ...
    fig.suptitle(
        f"Group 13-16 elements in periodic-table layout: least-spread point with {method} fitting",
        fontsize=15, y=0.995
    )
...
def main():
    ...
    all_points_dict = {}
    for element in PBLOCK_ELEMENTS:
        try:
    ...
    out_lines = OUT_DIR / "group13_16_periodic_layout_XO_fitted_lines_all_methods.csv"
    out_lsp = OUT_DIR / "group13_16_periodic_layout_XO_least_spread_points_all_methods.csv"

    all_lines_df.to_csv(out_lines, index=False)
    all_lsp_df.to_csv(out_lsp, index=False)

    # Generate one figure for each method
    for method in METHODS:
        out_fig = OUT_DIR / f"group13_16_periodic_layout_XO_least_spread_{method}.png"
        plot_pblock_elements_periodic_layout(
            method, all_points_dict, all_lines_df, all_lsp_df, out_fig
        )
