...
OUT_DIR = Path("out/5_alpha12_analysis_group13_clean")
...
def load_base_data():
    ...
    # Keep only Al, Ga, In
    df = df[df["element"].isin(GROUP13)].copy()
    if df.empty:
        raise ValueError("No rows found for selected clean Group 13 elements: Al, Ga, In")
...
def run_global_group_element_fits(df, filename):
    ...
    sub = df[df["group"] == "Group13"].copy()
    if len(sub) >= 2:
        out = fit_beta0_vs_beta_for_subset(sub)
        if out is not None:
            rows.append({"level": "group", "name": "Group13", **out})
...
def plot_groupwise_ratio_fits(df, out_path):
    ...
    if len(sub) >= 2:
        ...
        plt.scatter(sub["ionic_radius"], sub["beta0_over_beta"], s=70, alpha=0.85, edgecolors="black", label="Group13")
        model = LinearRegression()
        model.fit(x, y)
    ...
    plt.xlabel("ionic radius (Å)")
    plt.ylabel("beta0 / beta")
    plt.title("Group13 beta0 / beta vs ionic radius")
    plt.legend(frameon=False)
    plt.tight_layout()
    plt.savefig(out_path, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    print(f"[INFO] Elements: {sorted(df_no_radius['element'].unique())}")
    ...
    for model_name, feats in model_specs_round1:
        ...
        plot_pred_vs_true(
            df_no_radius,
            pred,
            f"{model_name}\nR2={res['R2']:.4f}, RMSE={res['RMSE']:.4f}",
            OUT_DIR / "plots_pred_vs_true" / f"{model_name}_pred_vs_true.png"
        )
    ...
    plot_pred_vs_true(
        df_no_radius,
        pred_int,
        f"{interaction_name}\nR2={res_int['R2']:.4f}, RMSE={res_int['RMSE']:.4f}",
        OUT_DIR / "plots_pred_vs_true" / f"{interaction_name}_pred_vs_true.png"
    )
    ...
    print(f"[INFO] Radius elements: {sorted(df_radius['element'].unique())}")
    ...
    plot_pred_vs_true(
        df_radius,
        pred_int_r,
        f"{interaction_name_radius}\nR2={res_int_r['R2']:.4f}, RMSE={res_int_r['RMSE']:.4f}",
        OUT_DIR / "plots_pred_vs_true" / f"{interaction_name_radius}_pred_vs_true.png"
    )
    ...
    plot_pred_vs_true(
        df_ratio["beta0_over_beta"].to_numpy(),
        ratio_pred,
        f"beta0 / beta ~ ionic radius\nR2={ratio_result['R2']:.4f}, RMSE={ratio_result['RMSE']:.4f}",
        OUT_DIR / "plots_ratio" / "ratio_pred_vs_true.png",
        xlabel="True beta0 / beta",
        ylabel="Predicted beta0 / beta"
    )
    ...
    for name, feats in [
        ...
        plot_pred_vs_true(
            df_ratio["ratio_resid"].to_numpy(),
            pred,
            f"{name}\nR2={out['R2']:.4f}, RMSE={out['RMSE']:.4f}",
            OUT_DIR / "plots_residuals" / f"{name}_pred_vs_true.png",
            xlabel="True residual",
    ...
    sub = df_radius[df_radius["group"] == "Group13"].copy()
    fit_out, _ = fit_ratio_vs_radius_for_subset(sub)
    if fit_out is not None:
        universality_rows.append({"level": "group", "name": "Group13", **fit_out})
